...
addons="WEBIF WEBIF_LIVELOG WEBIF_JQUERY WEBIF_COMPRESS WITH_SSL HAVE_DVBAPI WITH_EXTENDED_CW WITH_NEUTRINO READ_SDT_CHARSETS CS_ANTICASC WITH_DEBUG MODULE_MONITOR WITH_LB CS_CACHEEX CS_CACHEEX_AIO CW_CYCLE_CHECK LCDSUPPORT LEDSUPPORT CLOCKFIX IPV6SUPPORT WITH_ARM_NEON WITH_EMU WITH_SOFTCAM"
...
list_config() {
	...
	enabled_any MODULE_GBOX WEBIF_COMPRESS && echo "CONFIG_LIB_MINILZO=y" || echo "# CONFIG_LIB_MINILZO=n"
...
menu_addons() {
	${DIALOG} --checklist "\nChoose add-ons:\n " $height $width $listheight \
		...
		WEBIF_COMPRESS		"Compress webpages"						$(check_test "WEBIF_COMPRESS") \
